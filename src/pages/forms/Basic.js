...
export const CompulsorySelectLabels = ({ disabled,multi, options,change,value,headerName }) => {
...
export const SelectLabels = ({ disabled,multi, options,change,value,headerName }) => {
    const { t } = useTranslation();
    return (
        <div>
        <h5>{t(headerName)}</h5>
...
export const MultiSelectLabelsReact = ({ multi, options,id ,handleChange,value,placeholder}) => {
    
    const table = useSelector(state=>state.ActivityLimit.ownersTableDataMain);
    ...
    return (
        ...
        <Select
            ...
            onChange={(e)=>handleChange(e,id,table)}
        />
        </>
    )
}

export const MultiSelectLabels = ({ options,value,change,headerName }) => {
    const { t } = useTranslation();
    return (
        <div>
...
export const MultiSelectReact = ({ options,value,change,placeholder }) => {
    const { t } = useTranslation();
    return (
        <>
        <MultiSelect
...
export const Switches = ({ changeSwitch, switchLeftRight, switchName }) => {
    const { t } =useTranslation();
    return (
        <React.Fragment>
            <Form >
                <div style={{ display: 'flex' }}>
                    <span className='switch-left'> {t(switchLeftRight[0])} </span>
                    <Form.Check type="switch" id="custom-switch" onChange={() => changeSwitch(switchName)} />
                    <span className={`${`switch-right ${switchName}`}`}> {t(switchLeftRight[1])} </span>
                </div>
            </Form>
        </React.Fragment>
    );
...
export const Switches2 = ({ changeSwitch, switchName,disabled, id ,checked }) => {
    return (
        <React.Fragment>
            <Form >
                <div style={{ display: 'flex' }}>
                    <Form.Check  defaultChecked={checked} disabled={disabled} style={{margin:'auto'}} type="switch" id="custom-switch" onChange={() => changeSwitch(switchName,id)} />
                </div>
            </Form>
        </React.Fragment>
    );
...
export const CustomRadios = ({data,change}) => {
    ...
    return (
        <Form.Group  className="mt-2">
            <div>
                {
                    data?.map(data=>(
                        <Form.Check defaultChecked={data.defaultCheck} inline type="radio" name={data.name}  id={data.id} label={t(data.label)}  onClick={(e)=>change(e.target.id,data.name)} />
                    ))
                }
            </div>
        </Form.Group>
