...
const Body = ({ selectedItem, globalBrandFilterOptions, isClickAdd, getFilterData, setShowAddModal, setAddButtonDisableStatus, setIsClickAdd, companyIdForFilter }) => {
    ...
    // const [place, setPlace] = useState({ value: selectedItem.item.placeId, label: selectedItem.item.placeName });
    const [place, setPlace] = useState();

    const [placeOptions, setPlaceOptions] = useState([]);

    // const [placeType, setPlaceType] = useState({ value: selectedItem.item.placeTypeId, label: selectedItem.item.placeTypeName });
    const [placeType, setPlaceType] = useState();
    const [placeTypeOptions, setPlaceTypeOptions] = useState([]);

    // const [typeOfPriority, setTypeOfPriority] = useState({ value: selectedItem.item.typeOfPriorityId, label: selectedItem.item.typeOfPriorityName });
    const [typeOfPriority, setTypeOfPriority] = useState();

    const [typeOfPriorityOptions, setTypeOfPriorityOptions] = useState([]);

    // const [clientType, setClientType] = useState({ value: selectedItem.item.clientTypeId, label: selectedItem.item.clientTypeName });
    ...
    const [globalBrandOptions, setGlobalBrandOptions] = useState([...globalBrandFilterOptions]);
    ...
    useEffect(() => {
        if (isClickAdd) {
            ...
            FetchApiPost('services/Pages/Need/CreateNeed', 'POST', body).then((res) => {
                if (res.status === 201) {
                    res.json().then(({ data }) => {
                        handleUpdateDocument(data[0].id);
                    });
                } else if (res.status === 500 || res.status === 502) {
                    history.push('/error-500');
                } else {
    ...
    const handleUpdateDocument = (id) => {
        const fd = new FormData();
            ...
            xhr.open(
                'POST',
                `http://178.18.200.171:5000/services/Pages/Need/AddFilesToNeed`,
                true
            )
            xhr.send(fd);
                    ...
                    <div className='visit-content-need-add__files'>
                        {
                            document.map((item, index) => (
                                <div className='visit-content-need-add__files-container'>
                                    ...
                                    <div className='visit-content-need-add__files-item-delete' onClick={() => removeFile(item)}><Icon path={mdiDeleteOutline} size={1} /></div>
                                    </div>
                                </div>
                            ))
                        }
