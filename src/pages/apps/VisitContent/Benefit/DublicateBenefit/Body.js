...
const Body = ({ selectedItem, isClickAdd, getFilterData, setShowAddModal, setAddButtonDisableStatus, setIsClickAdd, companyIdForFilter }) => {
    ...
    }, [history, globalBrand, place, placeType, typeOfPriority, clientType]);
    ...
    useEffect(() => {
        if(isClickAdd){
            ...
            FetchApiPost('services/Pages/Benefit/CreateBenefit', 'POST', body)
                .then(res => {
                    if(res.status === 201){
                        res.json().then(({data}) => {
    ...
    const handleUpdateDocument = (id) => {
        const fd = new FormData();
            ...
            xhr.open(
                'POST',
                `http://178.18.200.171:5000/services/Pages/Benefit/AddFilesToBenefit`,
                true
            )
            xhr.send(fd);
