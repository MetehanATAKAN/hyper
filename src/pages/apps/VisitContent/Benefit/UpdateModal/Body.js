...
const Body = ({ selectedItem, isClickAdd, getFilterData, setShowAddModal, setAddButtonDisableStatus, setIsClickAdd, companyIdForFilter }) => {
    const { t } = useTranslation();
    const history = useHistory();
    console.log(selectedItem)
    let disableSelects = selectedItem.item.isLink === 1
    ...
    const [need, setNeed] = useState({ value: selectedItem.item.need.id, label: selectedItem.item.need.needName });
    const [needOptions, setNeedOptions] = useState([]);

    const [benefit, setBenefit] = useState(selectedItem.item.benefitName);
            ...
            FetchApiPost('services/AdminPanel/Defination/GetDefinationsByHeaderId', 'POST', {
                ...
                if(res.status === 200){
                    res.json().then(({ data }) => {
                        ...
                        let d = data.find(i => i.definationId === selectedItem.item.typeOfPriortiyId)
    ...
    useEffect(() => {
        ...
        FetchApiPost(`services/AdminPanel/Header/GetHeadersForClient`, 'POST', {
            ...
            }, [history, globalBrand, selectedItem, place, placeType, clientType, typeOfPriority])
            ...
            useEffect(() => {
                ...
                if(!profile) return;
            if(globalBrand && clientType && specialization.length === 0 || !place || !placeType || !typeOfPriority) return;
    ...
    useEffect(() => {
        if(isClickAdd){
            const body = {
                ...
                specializationIds: specialization.map(item => item.value),
                modifiedBy: localStorage.getItem('userName'),
                validStatus: selectedItem.item.validStatus,
            }
            FetchApiPost('services/Pages/Benefit/UpdateBenefit', 'POST', body)
                .then(res => {
                    if(res.status === 201){
                        res.json().then(({data}) => {
    ...
    return (
        <div className="visit-content-need-add">
            ...
            <MultipleSelects
                ...
                disabled={disableSelects ? true : clientType ? false : true}
            />
