...
const Delete = ({ modalShow, setModalShow, item, getFilterData }) => {
    ...
    const deleteBenefit = () => {
        ...
        FetchApiPost('services/Pages/PromoSubject/DeletePromoSubject', 'POST', deletedData).then((res) => {
            if (res.status === 201) {
                getFilterData();
                setModalShow(false);
            }
                    ...
                    <Modal.Body>
                        <div className="alert-modal-items">
                            ...
                            <div className="alert-modal-question">
                                {t('Are you sure you want to delete this promo subject?')} <br /> "{item.name}"
                            </div>
