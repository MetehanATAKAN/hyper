...
const WorkPlaceTable = () => {
    ...
    const columns = useMemo(
        ...
        () => [
            ...
            {
                accessorKey: 'microzoneName',
                header: t('Microzone Name'),
                size: '75',
            },
        ],
        []
    ...
    const calculate = () => {
        ...
        if (data1?.length !== 0) {
            ...
            FetchApiPost('services/Daywork/MicroTarget/CorrectionWorkPlacePlan', 'POST', body).then((res) =>
                (async () => {
                    try {
    ...
    const save = () => {
        if (isClickSave) {
            ...
            if (data1?.length !== 0) {
                setSaveLoading(true);
                FetchApiPost('services/Daywork/MicroTarget/CreateWorkPlacePlan', 'POST', body).then((res) =>
                    (async () => {
                        try {
                            if (res.status === 200 || res.status === 201) {
