...
import { mdiCheckCircleOutline, mdiClose, mdiSendCheck } from '@mdi/js';
import Icon from '@mdi/react';
import FailModal from '../../../../components/FailModal';
import { FetchApiPost } from '../../../../utils/http.helper';
import { useHistory } from 'react-router-dom';
const ActionModal = (props) => {
    const { modalShow, setModalShow, item, data, setData } = props;
    console.log(item)
    ...
    const handleClick = () => {
        const payload = {
            ...
            isApproved: item.statusId === 2 && 2,
            estimatedMunite: 0,
            modifiedBy: createdBy,
        };
        FetchApiPost('services/SurveySystem/Survey/UpdateSharedSurvey', 'POST', payload).then((res) => {
            if (res.status === 201) {
                let newData = data.map((i) => {
                    if (i.id === Number(item.item.id)) {
                        return {
                            ...i,
                            isApproved: 2
                        };
                        ...
                        <div className="alert-modal-items">
                            <div className="alert-modal-icon" style={{ marginTop: '2px' }}>
                                {item.statusId === 1 && ( // redact
                                    <Icon path={mdiClose} size={3} color="#6C757D" />
                                )}
                                {item.statusId === 2 && ( // CANCELED
                                    <Icon path={mdiClose} size={3} color="#FF679B" />
                                )}
                            ...
                            <div className="alert-modal-question">
                                {t(
                                    `Are you sure you want to send this survey for ${
                                        item.statusId === 1
                                            ? 'redact'
                                            : item.statusId === 2
                                            ? 'canceled'
