...
const Delete = ({ modalShow, setModalShow, item, data, setData }) => {
    ...
    const deleteShare = () => {
        ...
        FetchApiPost('services/SurveySystem/Survey/DeleteSharedSurvey', 'POST', deletedData).then((res) => {
            if (res.status === 201) {
                const newData = data.filter((item) => item.id !== deletedData.id);
                setData(newData);
                setModalShow(false);
                    ...
                    <Modal.Body>
                        <div className="alert-modal-items">
                            ...
                            <div className="alert-modal-question">
                                {t('Are you sure you want to delete this share?')} <br /> "{item.item.survey.surveyName}"
                            </div>
