...
const AddPromoSubject = ({ showModal, setShowModal, profile, setPage4TableData }) => {
    ...
    const { profileId, typeId, need, needId, benefitId, promoSubject, promoSubjectId } = profile;
    const [selectNeed, setSelectNeed] = useState({ value: needId, label: need });
    const [selectBenefit, setSelectBenefit] = useState([]);
    const [selectPromoSubject, setSelectPromoSubject] = useState({ value: promoSubjectId, label: promoSubject });
        ...
        FetchApiPost('services/Pages/PromoStrategy/GetPromoStrategyBenefit', 'POST', body).then((res) => {
            if (res.status === 200 || res.status === 201) {
                res.json().then(({ data }) => {
                    ...
                    const defaultBenefit = benefits.filter((x) => benefitId.includes(x.value));
                    if (data.length === 1 && defaultBenefit.length === 0) {
                    ...
                    setSelectBenefit(defaultBenefit);
                    ...
                    res.json().then(({ data }) => {
                        if (data.length === 1) {
                            setOptionsPromoSubject([
                                { value: data[0].promoSubjectId, label: data[0].promoSubjectName },
                            ]);
        ...
        <GlobalModal
            ...
            footer={
                <>
                    ...
                    <Button onClick={handleSelectPromoSubject} variant="warning">
                        {t('Update')}
                    </Button>
                </>
            }
