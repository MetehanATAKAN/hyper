...
const Filter = ({
        ...
        FetchApiPost('services/AdminPanel/Defination/GetDefinationsByHeaderIdandDefIds', 'POST', data).then((res) => {
            if (res.status === 200 || res.status === 201) {
                res.json().then(({ data }) => {
                    ...
                    setSelectPlace({ value: place[0].definationId, label: place[0].definationName });
                    setSelectPlaceType({ value: placeType[0].definationId, label: placeType[0].definationName });
                    setSelectTypeOfPriority({
                        value: typeOfPriorit[0].definationId,
                        label: typeOfPriorit[0].definationName,
                    });
                });
            }
            if (res.status === 500 || res.status === 502) {
    ...
    useEffect(() => {
        ...
        const condition = [!selectBusUnit ? true : false, !selectYear ? true : false, !selectSpec ? true : false];
        if (condition.some((x) => x === true)) return;
        const data = {
            year: selectYear?.value,
            BusId: selectBusUnit?.value,
            Quarters: [1, 2, 3, 4],
            SpecIds: [selectSpec.value],
        };
            ...
            .then((res) => {
                if (res.status === 200 || res.status === 201) {
                    res.json().then((data) => {
                        ...
                        setSelectGlobalBrand(products[0] ?? []);
