...
const EditCyclePeriod = ({
    ...
    useEffect(() => {
        ...
        FetchApiPost(
            ...
            if (res.status === 200 || res.status === 201) {
                res.json().then(({ data }) => {
                    setOptionsPeriod(data?.map((el) => ({ value: el.id, label: el.cycleName })));
                });
            }
    ...
    const handleChange = (value, obj) => {
        setCyclePeriod(obj);
        setName(obj.label.split(' [')[0]);
    };
    return (
        <>
            <GlobalModal
                header={t('Cycle Period')}
                showModal={showModal}
                ...
                footer={
                    <>
                        ...
                        <Button onClick={saveMeetingBtn} disabled={disableSaveBtn} variant="warning">
                            {t('Edit')}
                        </Button>
                    </>
                }
...
export default EditCyclePeriod;
