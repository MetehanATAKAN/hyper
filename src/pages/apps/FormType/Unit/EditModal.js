...
const EditModal = ({ showModal, setShowModal, itemId, getData }) => {
    const { t } = useTranslation();
    ...
    useEffect(() => {
        FetchApiGet(`services/Material/Unit/GetUnitById?id=${itemId}`, 'GET').then((res) => {
            if (res.status === 200) {
                res.json().then(({ data }) => {
    ...
    const handleUpdateButton = () => {
        ...
        FetchApiPost('services/Material/Unit/UpdateUnit', 'POST', data).then((res) => {
