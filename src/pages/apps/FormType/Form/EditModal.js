...
const EditModal = ({ showModal, setShowModal, getData, itemId }) => {
    const { t } = useTranslation();
    ...
    useEffect(() => {
        ...
        FetchApiGet(`services/Material/Form/GetFormById?id=${itemId}`, 'GET').then((res) => {
            if (res.status === 200) {
                res.json().then(({ data }) => {
                    setForm(data.formName);
                    setSelectFormType({ value: data.formTypeId, label: data.formTypeName });
                    setColor(data.color);
                    setAbb(data.formAbbreviation);
                });
            } else if (res.status === 500 || res.status === 502) {
                history.push('/error-500');
            }
    ...
    const handleUpdateButton = () => {
        ...
        FetchApiPost('services/Material/Form/UpdateForm', 'POST', data).then((res) => {
