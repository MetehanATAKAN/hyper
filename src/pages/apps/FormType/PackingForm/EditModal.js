...
const EditModal = ({ showModal, setShowModal, applyFilter, itemId }) => {
    ...
    useEffect(() => {
        FetchApiGet(`services/Material/PackingForm/GetPackingFormById?id=${itemId}`, 'GET').then((res) => {
            if (res.status === 200 || res.status === 201) {
                res.json().then(({ data }) => {
                    dispatch({
                        type: 'SET_MAIN_FORM',
                        payload: { value: data.mainFormId, label: data.mainFormName },
                    });
                    dispatch({
                        type: 'SET_INSIDE_ONE_CASSETE',
                        payload: data.mainInsideOneCassette,
                    });
                    dispatch({
                        type: 'SET_CASSETE_QUANTITY',
                        payload: data.mainCassetteQuantity,
                    });
                    dispatch({
                        type: 'SET_ASSIST_FORM',
                        payload: { value: data.assistFormId, label: data.assistFormName },
                    });
                    dispatch({
                        type: 'SET_INSIDE_ONE_CASSETE_TWO',
                        payload: data.assistInsideOneCassette,
                    });
                    dispatch({
                        type: 'SET_CASSETE_QUANTITY_TWO',
                        payload: data.assistCassetteQuantity,
                    });
                });
            }
            if (res.status === 500 || res.status === 502) {
    ...
    const handleAddButton = () => {
        const condition = [
            ...
            state.insideOneCassete === '',
            state.casseteQuantity === '',
            !state.selectAssistForm ? true : false,
            state.insideOneCasseteTwo === '',
            state.casseteQuantityTwo === '',
        ];
        ...
        const data = {
            ...
            mainInsideOneCassette: Number(state.insideOneCassete),
            mainCassetteQuantity: Number(state.casseteQuantity),
            assistFormId: state.selectAssistForm.value,
            assistInsideOneCassette: Number(state.insideOneCasseteTwo),
            assistCassetteQuantity: Number(state.casseteQuantityTwo),
            modifiedBy: user,
        };
        FetchApiPost('services/Material/PackingForm/UpdatePackingForm', 'POST', data).then((res) => {
            ...
            <GlobalModal
                ...
                footer={
                    <>
                        ...
                        <Button variant="warning" onClick={handleAddButton}>
                            {t('update')}
                        </Button>
                    </>
                }
