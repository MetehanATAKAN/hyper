...
function NonPromo({ visitEvolationsNonPromo, setVisitEvolationsNonPromo, setButtonDisable}) {
      const { t } = useTranslation();
      const appStatus = useSelector(state => state.Calendar.appStatus);
      useEffect(() => {
        let addNonPromo=0;

        visitEvolationsNonPromo.forEach((item, index) => {
    ...
    const visitEvolationsNonPromoLength=visitEvolationsNonPromo.length;
    if(addNonPromo === visitEvolationsNonPromoLength) {
      setButtonDisable({ disable:false, message: null})
    }
    else {
      setButtonDisable({ disable:true, message: t('All non Promos need to be selected.')})
    }
      }, [visitEvolationsNonPromo])
    
      const changeColor = (row, column, color) => {
        let element = document.getElementsByClassName("visit-evulotion-non-promo-percent-hidden-bar");
        ...
        let span = document.getElementsByClassName("visit-evulotion-non-promo-percent-hidden-bar")[row].parentNode.children[1].children
        for (let i = 0; i <= 5; i++) {
          if(i <= column){
      ...
      return (
          <div className='visit-evulotion'>
            ...
            <div className='visit-evulotion-container'>
              {
                visitEvolationsNonPromo?.map((item, index) => (
                  <div className='visit-evulotion-container__item' key={index}>
                    ...
                    <div className="visit-evulotion-non-promo-percent-hidden-bar"></div>
                  </div>
                ))
              }
            </div>
