...
function NonPromo({ setButtonDisable, setResetButton, resetButton, loyaltyNonPromo, setLoyaltyNonPromo, product }) {
  const appStatus = useSelector(state => state.Calendar.appStatus);
  const { t } = useTranslation();
  const eventId = useSelector(state => state.Calendar.eventId);
  useEffect(() => {
    if(resetButton.buttonStatus === true && resetButton.loyaltyName === 'LoyaltyNonPromo'){
      setResetButton({buttonStatus: false, loyaltyName: null})
      let buttonCount = 0;
      let element = document.getElementsByClassName("loyalty-product-non-promo-container__item");
      let brandElement = document.getElementsByClassName("loyalty-product-non-promo-percent-hidden-bar")
      loyaltyNonPromo?.forEach((item, itemIndex) => {
        item.data[10] !== 0 && (buttonCount = 1 + buttonCount)
        item.Profile?.forEach((prof, profileIndex) => {
          if(prof.percentage !== null){
            ...
            brandElement[itemIndex].style.width = `${(100 / 11) * (0 + 1)}%`
            brandElement[itemIndex].style.backgroundImage = "repeating-linear-gradient(45deg, #FA5C7C 0px, #FA5C7C 5px, #FA5C7C 5px, #FB7D96 5px, #FB7D96 10px)"
            item.percentage = 0;
      ...
      if(buttonCount > 0){
        setButtonDisable({ disable: true, message: t('All non Promos need to be selected.') })
      }else{
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if(appStatus !== 4){
        ...
        await loyaltyNonPromo?.map(globalSku => (
        ...
        await loyaltyNonPromo?.map((item,index) => {
            let totalNumber = 0
        ...
        // try{
        //   let deleteZeroValues = await loyaltyNonPromo.map(profiles =>({Profile: profiles.Profile.filter(item => item.data[10] !== 0 || item.data[10] !== null), BrandId: profiles.BrandId,
        //   GlobalSkuId: profiles.GlobalSkuId, GlobalSkuName: profiles.GlobalSkuName, data: profiles.data, isPromoOrOther: profiles.isPromoOrOther, percentage: profiles.percentage, profileIncrement: profiles.profileIncrement}))
        //   console.log(deleteZeroValues);
        //   setLoyaltyNonPromo(deleteZeroValues);
  ...
  const handleTotalPromoPercentSet = async (totalPercentIncreaseValues) => {
        let totalPercentDatas = [];
         for (let i = 0; i < totalPercentIncreaseValues.length; i++) {
           ...
           for (let x = 0; x < 10; x++) {
            if((Number(totalPercentData[x]) === totalPercentData[x]) && (totalPercentData[x] % 1 !== 0)){
             totalPercentData.push(Number((totalPercentData[x] + totalPercentIncreaseValues[i]).toFixed(2)))
            }else{
         ...
         setLoyaltyNonPromo(state => state.map((item, index) => ({BrandId: item.BrandId, GlobalSkuId: item.GlobalSkuId, GlobalSkuName: item.GlobalSkuName, profileIncrement: (totalPercentDatas[index][10] !== 0 && item.profileIncrement === null) ? 0 : item.profileIncrement, Profile: item.Profile, 
  ...
  useEffect(() => {
    let countForButton = 0;
    let loyaltyOtherLength = 0
    if(appStatus !== 4){
      loyaltyNonPromo?.map((brand) => (
        brand.data !== null && (brand.data[10] !==0 && (
          brand.Profile?.map((profile) => (
            profile.data !== null && (loyaltyOtherLength = loyaltyOtherLength + 1)
          ))
        ))
    ))
    }
    ...
    if(appStatus !== 4){
      if (countForButton === loyaltyOtherLength) {
        setButtonDisable({ disable: false, message: null })
      }
      else {
        setButtonDisable({ disable: true, message: t('All non Promos need to be selected.') })
  ...
  const changeAccordionColor = (accordionIndex, percent, color) => {
    try {
      let accordionElement = document.getElementsByClassName("loyalty-product-non-promo-percent-hidden-bar");
  ...
  const handleClick = (event, percent, accordionIndex, profileIndex, data, profileId) => {
    if(appStatus !== 4){
      loyaltyNonPromo?.map((item, index) => {
  ...
  return (
    <div className='loyalty-product'>
      <div className='loyalty-product__headers nav-tabs'>
        ...
        <div className='nav-item nav-link active'>{t('Non- Promo')}</div>
        <div className='nav-item'>{t('Other')}</div>
      </div>
      <div className='loyalty-product-accordion-container'>
      <Accordion defaultActiveKey="0">
      {
        loyaltyNonPromo?.map((accordion, accordionIndex) => (
          <>
            {
              ((accordion.data !== null) && (accordion.data[10] !== 0)) ? (
                  ...
                  <Accordion.Item eventKey={accordionIndex}>
                    <Accordion.Header>
                      <div className='loyalty-product-accordion-header-container'>
                        <div className='loyalty-product-accordion-header-container__head'>
                          <div className='loyalty-skuname'>{accordion.GlobalSkuName}</div>
                          <div>{accordion.percentage === null ? 0 : accordion.percentage}%</div>
                        </div>
                        <div className='loyalty-product-non-promo-accordion-container__bar'>
                          {
                            accordion?.data.map((totalItem, totalIindex) => (
                              <span key={totalIindex} >{totalItem}</span>
                            ))
                          }
                        <div className="loyalty-product-non-promo-percent-hidden-bar"></div>
                        </div>
                      </div>
                    </Accordion.Header>
                    <Accordion.Body>
                        <div className='loyalty-product-non-promo-container'>
                          <div className='loyalty-product-non-promo-container__item'>
                              {
                                  ...
                                  <>
                                    {
                                      ...
                                      <div>
                                        <div className='loyalty-product-non-promo-container__header'>
                                        ...
                                        <div className="loyalty-product-non-promo-container__bar" >
                                          { 
                                        ...
                                        <div className="loyalty-non-promo-percent-hidden-bar" ></div>
                    ...
                    <div className='loyalty-product-non-promo-container__item'></div>
                    <div className="loyalty-product-non-promo-percent-hidden-bar"></div>
                  </div>
            }
          </>
