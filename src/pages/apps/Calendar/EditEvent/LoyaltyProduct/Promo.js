...
function Promo({ setButtonDisable, setResetButton, resetButton, loyaltyPromo, setLoyaltyPromo, product }) {
  const appStatus = useSelector(state => state.Calendar.appStatus);
  const { t } = useTranslation();
  const eventId = useSelector(state => state.Calendar.eventId);
  useEffect(() => {
    if(resetButton.buttonStatus === true && resetButton.loyaltyName === 'LoyaltyPromo'){
      setResetButton({buttonStatus: false, loyaltyName: null})
      let buttonCount = 0;
      let element = document.getElementsByClassName("loyalty-product-promo-container__item");
      let brandElement = document.getElementsByClassName("loyalty-product-promo-percent-hidden-bar")
      loyaltyPromo?.forEach((item, itemIndex) => {
  ...
  useEffect(() => {
    (async () => {
      if(appStatus !== 4){
        await loyaltyPromo?.map(globalSku => (
        ...
        await loyaltyPromo?.map((item,index) => {
            let totalNumber = 0
  ...
  const handleTotalPromoPercentSet = async (totalPercentIncreaseValues) => {
        let totalPercentDatas = [];
         for (let i = 0; i < totalPercentIncreaseValues.length; i++) {
           ...
           for (let x = 0; x < 10; x++) {
             if((Number(totalPercentData[x]) === totalPercentData[x]) && (totalPercentData[x] % 1 !== 0)){
              totalPercentData.push(+(  totalPercentIncreaseValues[i] + totalPercentData[x]).toFixed(2))
         ...
         setLoyaltyPromo(state => state.map((item, index) => ({BrandId: item.BrandId, GlobalSkuId: item.GlobalSkuId, GlobalSkuName: item.GlobalSkuName, profileIncrement:(totalPercentDatas[index][10] !== 0 && item.profileIncrement === null) ? 0 : item.profileIncrement , Profile: item.Profile, 
  ...
  useEffect(() => {
    ...
    if(appStatus !== 4){
      loyaltyPromo?.map((brand) => (
        brand.data !== null && (brand.data[10] !==0 && (
          brand.Profile?.map((profile) => (
            profile.data !== null && (loyaltyOtherLength++)
          ))
        ))
    ))
    }
  ...
  const changeAccordionColor = (accordionIndex, percent, color) => {
    try{
      let accordionElement = document.getElementsByClassName("loyalty-product-promo-percent-hidden-bar");
  ...
  return (
    <div className='loyalty-product'>
      ...
      {
        loyaltyPromo?.map((accordion, accordionIndex) => (
          <>
            {
              ((accordion.data !== null) && (accordion.data[10] !== 0)) ? (
                  ...
                  <Accordion.Item eventKey={accordionIndex}>
                    <Accordion.Header>
                      <div className='loyalty-product-accordion-header-container'>
                        ...
                        <div className='loyalty-product-promo-accordion-container__bar'>
                          {
                            accordion?.data.map((totalItem, totalIindex) => (
                              <span key={totalIindex} >{totalItem}</span>
                            ))
                          }
                        <div className="loyalty-product-promo-percent-hidden-bar"></div>
                        </div>
                      </div>
                    </Accordion.Header>
                    <Accordion.Body>
                        <div className='loyalty-product-promo-container'>
                          <div className='loyalty-product-promo-container__item'>
                              {
                                  ...
                                  <>
                                    {
                                      ...
                                      <div>
                                        <div className='loyalty-product-promo-container__header'>
                                        ...
                                        <div className="loyalty-product-promo-container__bar" >
                                          { 
                                        ...
                                        <div className="loyalty-promo-percent-hidden-bar" ></div>
                  ...
                  <div className='loyalty-product-promo-container__item'></div>
                  <div className="loyalty-product-promo-percent-hidden-bar"></div>
                </div>
            } 
          </>
