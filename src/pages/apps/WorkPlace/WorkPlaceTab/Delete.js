...
const Delete = ({ modalShow, setModalShow, item, getFilterData }) => {
    ...
    const deleteBenefit = () => {
        const deletedData = {
            ids: item.length === undefined ? [item.id] : item,
            modifiedBy: modifiedBy,
        };
        FetchApiPost('services/CRM/WorkPlace/DeleteWorkPlace', 'POST', deletedData).then((res) => {
            if (res.status === 201) {
                getFilterData();
                setModalShow(false);
            }
                    ...
                    <Modal.Body>
                        <div className="alert-modal-items">
                            ...
                            <div className="alert-modal-question">
                                {item.length === undefined ? t('Are you sure you want to delete this work place?') : t('Are you sure you want to delete these work places?')} <br /> 
                                {item.length === undefined && `"${item.name}"`}
                            </div>
