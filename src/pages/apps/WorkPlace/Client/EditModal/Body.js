...
const Body = ({ clientTypeName, selectedItem, setNextButtonDisableStatus, setAddButtonDisableStatus, modalTabValue, setModalTabValue, isClickAdd, setShow, getFilterData, setIsClickAdd }) => {
    ...
    const [clientType, setClientType] = useState({ value: selectedItem.clientType.definationId === 0 ? selectedItem.clientType.id : selectedItem.clientType.definationId, label: selectedItem.clientType.definationName });
    ...
    const [middleName, setMiddleName] = useState(selectedItem.middleName);

    const [specialization, setSpecialization] = useState({ value: selectedItem.specialization.specId, label: selectedItem.specialization.specName });
    const [specializationOptions, setSpecializationOptions] = useState([]);

    const [gender, setGender] = useState(selectedItem.gender === 'Male' ? { value: 1, label: t('Male')} : { value: 2, label: t('Female')});
    ...
    const [birthday, setBirthday] = useState(selectedItem.birthday);

    const [email, setEmail] = useState(selectedItem.email);
    const [workPhone, setWorkPhone] = useState(selectedItem.workPhone);
    const [mobilePhone, setMobilePhone] = useState(selectedItem.mobilePhone);

    const [degree, setDegree] = useState();
    const [degreeOptions, setDegreeOptions] = useState([]);

    const [keyOpinion, setKeyOpinion] = useState(selectedItem.keyOpinionLeader);
    const [decisionMaker, setDecisionMaker] = useState(selectedItem.decisionMaker);
    ...
    const [workPlace, setWorkPlace] = useState({value: selectedItem.workPlaces[0].defination1.definationId === 0 ? selectedItem.workPlaces[0].defination1.id : selectedItem.workPlaces[0].defination1.definationId, label: selectedItem.workPlaces[0].defination1.definationName });
    ...
    const [position, setPosition] = useState({ value: selectedItem.workPlaces[0].position.id, label: selectedItem.workPlaces[0].position.name });
    ...
    useEffect(() => {
        ...
        let zones = [];
        selectedItem.workPlaces.map(wp => {
            if(!zones.find(i => i.value === wp.zone.id)){
                zones.push({ value: wp.zone.id, label: wp.zone.name })
            }
        })
        setZone(zones)
            ...
            .then(res => {
                if(res.status === 200){
                    res.json().then(data => {
                        ...
                        let d = data.find(i => i.DegreeId === selectedItem.degreeId)
                        if(d){
                            setDegree({ value: d.DegreeId, label: d.DegreeName})
                        }
                ...
                <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: '1rem' }}>
                        <div>
                            ...
                            <DatePicker
                            defaultValue={moment(selectedItem.birthday)}
                                style={{
                                    width: '217px',
                                }}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', columnGap: '1rem' }}>
                        ...
                        <div style={{ display: 'grid' }}>
                            ...
                            <div
                                ...
                                <Checkbox checked={keyOpinion} onChange={(e) => setKeyOpinion(e.target.checked)} />
                        ...
                        <div style={{ display: 'grid' }}>
                            ...
                            <div
                                ...
                                <Checkbox checked={decisionMaker} onChange={(e) => setDecisionMaker(e.target.checked)} />
