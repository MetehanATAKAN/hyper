...
const Body = ({ setButtonDisableStatus, onClickAddBtn, setIsShow, setFilterData, selectedValue, filterData }) => {
    const { t } = useTranslation();
    const [roleName, setRoleName] = useState(selectedValue.roleName);
    ...
    useEffect(() => {
        setRoleName(selectedValue.roleName);
        if (selectedValue.scopeId === 0) {
            setGlobalCheck(true);
        } else if (selectedValue.scopeId === 1) {
            setRestrictedCheck(true);
        } else {
            setTerritoryCheck(true);
        }
    }, [selectedValue]);
    ...
    useEffect(() => {
        ...
        if (onClickAddBtn) {
            FetchApiPost('services/AuthorizationSystem/Role/UpdateRole', 'POST', {
                ...data,
            }).then((res) => {
                if (res.status === 201) {
                    res.json().then((data) => {
                        let newFilterData = filterData.map((item) => {
                            if (item.id === data.data.id) {
                                return {
                                    ...
                                    scopeId: data.data.scopeId,
                                    updateBy: data.data.modifiedBy,
                                    updateDate: data.data.modifiedDate,
                                };
                            } else {
                                return item;
                            }
