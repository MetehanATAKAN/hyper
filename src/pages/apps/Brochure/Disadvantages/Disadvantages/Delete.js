import React from 'react';
import { Modal } from 'react-bootstrap';
import { useTranslation } from 'react-i18next';
import { mdiAlertCircleOutline } from '@mdi/js';
import Icon from '@mdi/react';
// import '../../../../../assets/scss/custom/components/alertModal.scss';
import { FetchApiPost } from '../../../../../utils/http.helper';

const Delete = ({ modalShow, setModalShow, deleteItem, disadvantagesFilteredDatas, setDisadvantagesFilteredDatas }) => {
    const { t } = useTranslation();
    const modifiedBy = localStorage.getItem('userName');
    const onHide = () => {
        setModalShow(true);
    };

    const deleteNeed = () => {
        if (deleteItem.id === 0) return;
        const deletedData = {
            disadvantageId: Number(deleteItem.disadvantageId),
            modifiedBy: modifiedBy,
        };
        FetchApiPost('services/Pages/Disadvantage/DeleteDisadvantage', 'POST', deletedData).then((res) => {
            if (res.status === 201) {
                const filteredArr = disadvantagesFilteredDatas?.filter((el) => el.disadvantageId !== Number(deleteItem.disadvantageId));
                setDisadvantagesFilteredDatas(filteredArr);
                setModalShow(false);
            }
        });
    };

    return (
        <div className="alert-modal">
            <Modal size="md" centered show={modalShow} onHide={onHide} className="alert-modal">
                <Modal.Body>
                    <div className="alert-modal-items">
                        <div className="alert-modal-icon" style={{ marginTop: '2px' }}>
                            <Icon path={mdiAlertCircleOutline} size={3} color="#FA5C7C" />
                        </div>
                        <div className="alert-modal-title" style={{ marginTop: '14px' }}>
                            <span style={{ color: '#6C757D' }}>{t('Are you sure ?')}</span>
                        </div>
                        <div className="alert-modal-question">
                            "{deleteItem.disadvantageName}" {t('Are you sure you want to delete this disadvantage?')}
                        </div>
                        <div className="alert-modal-buttons" style={{ marginTop: '25px' }}>
                            <button
                                style={{
                                    color: '#fff',
                                    fontWeight: '600',
                                    width: '125px',
                                }}
                                className="delete"
                                onClick={deleteNeed}>
                                {t('Yes, delete it!')}
                            </button>
                            <button
                                style={{
                                    backgroundColor: '#EEF2F7',
                                    color: '#6C757C',
                                    fontWeight: '600',
                                    width: '75px',
                                }}
                                className="cancel"
                                onClick={() => setModalShow(false)}>
                                {t('cancel')}
                            </button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
        </div>
    );
};

export default Delete;
