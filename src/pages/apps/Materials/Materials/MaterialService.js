...
import MaterialServiceTable from './Tables/MaterialServiceTable';
const Filter = ({
    selectMainCategory,
    ...
    return (
        <div
            ...
            <MultipleSelects
                ...
                className="filter-radius"
                selectedItems={selectCategory3}
                setSelectedItems={setSelectCategory3}
                options={optionsCategory3}
                size="small"
                width="20%"
            />
...
const MaterialService = ({
    ...
    const tableData = data?.map((el, i) => ({
        ...
        categorySub2:
            el.categorySecondSubs.length === 0 ? (
                '-'
            ) : (
                <Tippy
                    placement="bottom"
        ...
        categorySub3:
            el.categoryThirdSubs.length === 0 ? (
                ' -'
            ) : (
                <Tippy
                    placement="bottom"
                    content={el.categoryThirdSubs?.map((x, indx) => (
                        <span>
                            {x.name}
                            {indx !== el.categoryThirdSubs.length - 1 && ', '}
                        </span>
                    ))}>
                    <span>
                        {el.categoryThirdSubs?.map((item, idx) => (
                            <span>
                                {item.name}
                                {idx !== el.categoryThirdSubs.length - 1 && ', '}
                            </span>
                        ))}
                    </span>
                </Tippy>
            ),
        materialService: el.name,
        abb: el.abb,
        ...
        materialType:
            el.types.length === 0 ? (
                ...
                <Tippy
                    placement="bottom"
                    content={el.types?.map((x, indx) => (
                        <span>
                            {x.name}
                            {indx !== el.types.length - 1 && ', '}
                        </span>
                    ))}>
                    <span>
                        {el.types?.map((item, idx) => (
                            <span>
                                {item.name}
                                {idx !== el.types.length - 1 && ', '}
                            </span>
                        ))}
                    </span>
                </Tippy>
                ...
                <Button
                    onClick={() =>
                        setInfo({
                            ...
                            thirdCategories: el.categoryThirdSubs?.map((x) => ({ value: x.id, label: x.name })),
                            types: el.types?.map((x) => ({ value: x.id, label: x.name })),
                        })
                    }
        ...
        <>
            <MaterialServiceTable
                ...
                filters={
                    <Filter
                        ...
                        setSelectCategory3={setSelectCategory3}
                        optionsCategory3={optionsCategory3}
                        statusArr={materialServiceFilter}
                        setCloseFilter={setCloseFilter}
                    />
                }
            />
            {showDeleteModal && (
                <Delete
                    ...
                    info={info}
                    url={'services/Material/MaterialOrServices/DeleteMaterialOrServices'}
                    applyFilter={applyFilter}
                />
            )}
        </>
