...
const Update = ({ handleUpdate, selectedValue, isShow, setIsShow }) => {
    ...
    const [duties, setDuties] = useState(selectedValue.name);
  ...
  return (
    <Modal show={isShow} onHide={toggle} size={'md'} >
                ...
                <Modal.Body>
                    ...
                    <Row>
                        <Form.Control type="text" placeholder={t('duties type')} value={duties} onChange={(e) => setDuties(e.target.value)} />
                    </Row>
                    <Row className='mt-3'>
                ...
                <Modal.Footer style={{ backgroundColor: '#FAFBFE ' }} >
                    ...
                    <Button
                        ...
                        onClick={() => handleUpdate(duties, description)}
                        disabled={
                            duties.trim().length === 0 || 
