...
const DutiesType = ({ handleAddModal, handleAddModalStatus, setHandleAddModalStatus }) => {
    ...
    useEffect(() => {
        if(handleAddModalStatus.clickAdd === true && handleAddModalStatus.tabName === 'Duties Type'){
    ...
    const [columnWidths, setColumnWidths] = useState([
        { columnName: 'dutiesType', width: "48%" },
    ...
    const itemsFromBackend = [
        { id: '1', isFreeze: false, content: 'Duties Type', columnName: 'dutiesType', width: 100 },
        { id: '2', isFreeze: false, content: 'Description', columnName: 'description', width: 100 }
    ];

    const [groupByItems, setGroupByItems] = useState([
        { id: '0', isShow: true, columnName: 'dutiesType', content: t('Duties Type') },
    ...
    const [columnOrders, setColumnOrders] = useState(['dutiesType', 'description', 'actions']);
    ...
    const tableData = filterData.map((item, index) => ({
        id: item.id,
        dutiesType: item.name,
        description: item.description,
        actions: action[index].content
    }))

    useEffect(() => {
        FetchApiPost('services/Material/DutiesType/GetAllDutiesType', 'GET')
        .then(res => {
            if(res.status === 200){
                res.json().then(data => (
    ...
    const handleClickDeleteButton = () => {
        FetchApiPost('services/Material/DutiesType/DeleteDutiesType', 'POST', {
            dutiesTypeId: deleteItem.id,
    ...
    const handleUpdate = (name, description) => {
        FetchApiPost('services/Material/DutiesType/UpdateDutiesType', 'POST', {
            id: updateItem.id,
            name: name,
