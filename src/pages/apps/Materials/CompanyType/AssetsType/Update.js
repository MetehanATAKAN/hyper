...
const Update = ({ handleUpdate, selectedValue, isShow, setIsShow }) => {
    ...
    const [assets, setAssets] = useState(selectedValue.name);
  ...
  return (
    <Modal show={isShow} onHide={toggle} size={'md'} >
                <Modal.Header onHide={toggle} closeButton style={{ backgroundColor: '#FFFFFF', color: '#000000' }} >
                    ...
                    >
                        {t('Update Assets Type')}
                    </h4>
                </Modal.Header>
                <Modal.Body>
                    <div className='mb-3' style={{padding: '16px'}} >
                    <Row>
                        <Col style={{fontWeight: 600, color: '#6c757d'}} className='p-0'>{t('assets type')}</Col>
                        <Col style={{fontSize: "1.2rem", color: 'red'}} className='p-0 text-end font-weight-bold'>*</Col>
                    </Row>
                    <Row>
                        <Form.Control type="text" placeholder={t('assets type')} value={assets} onChange={(e) => setAssets(e.target.value)} />
                    </Row>
                    <Row className='mt-3'>
                ...
                <Modal.Footer style={{ backgroundColor: '#FAFBFE ' }} >
                    ...
                    <Button
                        ...
                        onClick={() => handleUpdate(assets, description)}
                        disabled={
