...
const AssetsType = ({ handleAddModal, handleAddModalStatus, setHandleAddModalStatus }) => {
    ...
    useEffect(() => {
        if(handleAddModalStatus.clickAdd === true && handleAddModalStatus.tabName === 'Assets Type'){
    ...
    const [columnWidths, setColumnWidths] = useState([
        { columnName: 'assetsType', width: "48%" },
    ...
    const itemsFromBackend = [
        { id: '1', isFreeze: false, content: 'Assets Type', columnName: 'assetsType', width: 100 },
        { id: '2', isFreeze: false, content: 'Description', columnName: 'description', width: 100 }
    ];

    const [groupByItems, setGroupByItems] = useState([
        { id: '0', isShow: true, columnName: 'assetsType', content: t('Assets Type') },
    ...
    const [columnOrders, setColumnOrders] = useState(['assetsType', 'description', 'actions']);
    ...
    const tableData = filterData.map((item, index) => ({
        id: item.id,
        assetsType: item.name,
        description: item.description,
        actions: action[index].content
    }))

    useEffect(() => {
        FetchApiPost('services/Material/AssetsType/GetAllAssetsType', 'GET')
        .then(res => {
            if(res.status === 200){
                res.json().then(data => (
    ...
    const handleClickDeleteButton = () => {
        FetchApiPost('services/Material/AssetsType/DeleteAssetsType', 'POST', {
            assetsTypeId: deleteItem.id,
    ...
    const handleUpdate = (name, description) => {
        FetchApiPost('services/Material/AssetsType/UpdateAssetsType', 'POST', {
            id: updateItem.id,
            name: name,
