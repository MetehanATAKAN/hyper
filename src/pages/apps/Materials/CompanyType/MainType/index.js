...
const MainType = ({ handleAddModal, handleAddModalStatus, setHandleAddModalStatus }) => {
    ...
    useEffect(() => {
        if(handleAddModalStatus.clickAdd === true && handleAddModalStatus.tabName === 'Main Type'){
    ...
    const itemsFromBackend = [
        { id: '1', isFreeze: false, content: 'Main Type', columnName: 'mainType', width: 100 },
        { id: '2', isFreeze: false, content: 'Description', columnName: 'description', width: 100 }
    ];

    const [groupByItems, setGroupByItems] = useState([
        { id: '0', isShow: true, columnName: 'mainType', content: t('Main Type') },
    ...
    const [columnOrders, setColumnOrders] = useState(['mainType', 'description', 'actions']);
    ...
    useEffect(() => {
        FetchApiPost('services/Material/MainType/GetAllMainType', 'GET')
        .then(res => {
            if(res.status === 200){
                res.json().then(data => (
    ...
    const handleClickDeleteButton = () => {
        FetchApiPost('services/Material/MainType/DeleteMainType', 'POST', {
            mainTypeId: deleteItem.id,
    ...
    const handleUpdate = (name, description) => {
        FetchApiPost('services/Material/MainType/UpdateMainType', 'POST', {
            id: updateItem.id,
            name: name,
