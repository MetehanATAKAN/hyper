...
const Delete = ({ modalShow, setModalShow, selectedValue, filterData, setFilterData }) => {
    ...
    const deleteBtn = () => {
        FetchApiPost('services/Material/ManufacturerToSeller/DeleteManufacturerToSeller', 'POST', {
            id: selectedValue.id,
                        ...
                        <div className="alert-modal-question" style={{ marginTop: '25px' }}>
                            {error === '' ? (
                                <>
                                    ...
                                    <br />
                                    {t('manufacturer')}: "{selectedValue.manufacturer}" / {t('seller')}: "{selectedValue.sellerName}"
                                </>
                            ) : (
                                t(error)
                            )}
