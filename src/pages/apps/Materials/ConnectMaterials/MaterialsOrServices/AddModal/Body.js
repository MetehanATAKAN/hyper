...
const Body = ({
    ...
    const [selectedStandartMOQ, setSelectedStandartMOQ] = useState();
    ...
    const handleFile = async (id) => {
        if (coAFiles.length > 0) {
            ...
            for (let index = 0; index < coAFiles.length; index++) {
                fd.append('files', coAFiles[index].originFileObj, `${id}-${coAFiles[index].name}`);
            }
            ...
            xhr.onload = () => {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    // we done!
                    setFilterData((prev) => [...prev, response.data]);
                }
            };
            setShowModal(false);
        } else {
    ...
    useEffect(() => {
        if (isClickAdd) {
            ...
            const data = {
                ...
                moq:
                    selectedStandartMOQ !== undefined
                        ? Number(selectedStandartMOQ) === 0
                            ? 1
                            : Number(selectedStandartMOQ)
                        : 1,
                form: selectedForm,
            ...
            FetchApiPost('services/Material/MaterialOrServicesToCompany/CreateMaterialOrServicesToCompany', 'POST', {
                ...data,
            }).then((res) => {
                if (res.status === 201) {
                    res.json().then((data) => {
        ...
        <div className="material-or-services-add-modal">
            <Row className="ms-0 me-0">
                <Col
                    ...
                    <div>
                        <Form.Check type="switch" onChange={(e) => setRawMaterial(e.target.checked)} />
                    </div>
                </Col>
                <Col
                    className="d-flex justify-content-between"
                    style={{ background: '#FAFBFE', padding: '6px 8px', borderRadius: '4px', marginBottom: '0' }}>
                    <label>{t('sapmle request')}</label>
                    <div>
                ...
                <Col className={`p-0`}>
                    ...
                    <div className="upload-component">
                        <Upload multiple={true} onChange={(e) => handleNewUpload(e)} fileList={coAFiles}>
                            <Button style={{ color: '#6c757d' }} icon={<UploadOutlined />}>
                                {t('Click to Upload')}
                            </Button>
                        </Upload>
